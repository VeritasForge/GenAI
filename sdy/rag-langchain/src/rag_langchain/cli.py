"""CLI entrypoint for RAG pipeline (LangChain)."""
...
app = typer.Typer(help="RAG Pipeline CLI (LangChain)")


@app.command()
def hello() -> None:
    """Smoke test — 환경 설정이 정상인지 확인."""
    typer.echo("RAG pipeline is ready. (LangChain)")
...
def chat(
    ...
    typer.echo("RAG Chat - LangChain (type 'quit' or 'exit' to stop)")
