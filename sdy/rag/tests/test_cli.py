"""TDD tests for CLI commands (Step 7)."""
...
from rag.pipeline import AskResult, IndexResult
...
def _make_ask_result(**kwargs) -> AskResult:
    defaults = {
        ...
        "sources": [
            ...
            SearchResult(
                document=Document(
                    content="side effects include nausea",
                    metadata={"filename": "metformin.txt", "chunk_index": 7},
                ),
                score=0.48,
            ),
        ],
...
class TestBasicCommands:
    ...
    @patch("rag.cli.index_documents")
    def test_index_calls_pipeline(self, mock_index):
    ...
    @patch("rag.cli.ask_question")
    def test_ask_calls_pipeline(self, mock_ask):
...
class TestOutputFormat:
    @patch("rag.cli.index_documents")
    def test_index_output_shows_counts(self, mock_index):
        """Cycle 4: index output includes document and chunk counts."""
        mock_index.return_value = _make_index_result(
            total_documents=3, total_chunks=69
        )
    ...
    @patch("rag.cli.ask_question")
    def test_ask_output_shows_sources(self, mock_ask):
    ...
    @patch("rag.cli.ask_question")
    def test_ask_output_shows_answer(self, mock_ask):
...
class TestErrorHandling:
    @patch("rag.cli.index_documents")
    def test_index_bad_dir_error(self, mock_index):
    ...
    @patch("rag.cli.ask_question")
    def test_ask_runtime_error(self, mock_ask):
...
class TestIntegration:
    def test_full_index_via_cli(self, tmp_path):
